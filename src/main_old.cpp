...
int main(int argc, char* argv[]) {
    ...
    if (!config.getCurrentFile().empty() && fs::exists(config.getCurrentFile())) {
        ...
        // Trouver le nœud correspondant au fichier courant dans la playlist
        std::function<PlaylistNode*(PlaylistNode*, const std::string&)> findNodeByPath;
        findNodeByPath = [&](PlaylistNode* node, const std::string& filepath) -> PlaylistNode* {
            if (!node) return nullptr;
            
            // Vérifier si ce nœud correspond au fichier
            if (!node->isFolder && !node->filepath.empty()) {
                ...
                fs::path targetPath = fs::canonical(filepath);
            ...
            for (auto& child : node->children) {
                PlaylistNode* found = findNodeByPath(child.get(), filepath);
        ...
        PlaylistNode* foundNode = findNodeByPath(playlistRoot.get(), config.getCurrentFile());
        if (foundNode) {
            currentPlaylistNode = foundNode;
            std::cout << "Nœud trouvé pour le fichier courant: " << foundNode->name << std::endl;
        } else {
            std::cout << "Nœud non trouvé pour le fichier courant: " << config.getCurrentFile() << std::endl;
        }
    }
    
    // Flag pour indiquer qu'on doit scroller vers le nœud courant au premier rendu
    bool shouldScrollToCurrentNode = (currentPlaylistNode != nullptr);
    ...
    while (running) {
        ...
        // Panneau Playlist (droite)
        {
            ...
            // Fonction pour vérifier si un nœud est un parent du nœud courant
            std::function<bool(PlaylistNode*)> isParentOfCurrent = [&](PlaylistNode* node) -> bool {
                if (!node || !currentPlaylistNode) return false;
                PlaylistNode* parent = currentPlaylistNode->parent;
                while (parent) {
            ...
            std::function<void(PlaylistNode*, int)> renderNode = [&](PlaylistNode* node, int depth) {
                ...
                bool shouldOpen = isParentOfCurrent(node); // Ouvrir si c'est un parent du nœud courant
                
                if (node->isFolder) {
                    // Afficher un dossier avec TreeNode
                    // Ouvrir automatiquement si c'est un parent du nœud courant
                    ImGuiTreeNodeFlags flags = ImGuiTreeNodeFlags_OpenOnArrow;
                    if (isSelected) flags |= ImGuiTreeNodeFlags_Selected;
                    if (shouldOpen) flags |= ImGuiTreeNodeFlags_DefaultOpen;
                    ...
                    // Scroller vers le nœud courant au premier rendu
                    if (isCurrent && shouldScrollToCurrentNode) {
                        ImGui::SetScrollHereY(0.5f); // Centrer verticalement
                    }
            ...
            // Réinitialiser le flag après le premier rendu
            if (shouldScrollToCurrentNode) {
                shouldScrollToCurrentNode = false;
            }
